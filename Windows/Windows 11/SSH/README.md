...
| 🔒 SSH_Key_Generator | Generates and manages Ed25519 or RSA keys with custom naming, comments |
...
| 📂 **Auto-Saving** | Places keys directly into your `user/.ssh` folder with secure permissions set on first run |
| 📋 **Key Management** | List existing keys, view public keys, copy to clipboard, and display fingerprints |
...
| Feature | Details |
|:--- | :--- |
| 🗂️ **Config Sync** | Automatically reads, adds, edits, and removes entries from your Windows SSH config file |
...
| 🧹 **Known_Hosts Cleanup** | When removing a server, offers to wipe its old fingerprint from `known_hosts` |
| ✅ **Input Validation** | Validates port ranges, blocks shell-special characters, and confirms aliases before saving |
