...
# ════════════════════════════════════════════════════════════
# ssh_connection_manager.sh — Manage SSH servers via ~/.ssh/config
# ════════════════════════════════════════════════════════════
...
# Returns 0 for yes/y, 1 for anything else
ask_yes_no() {
    case "${1,,}" in y|yes) return 0 ;; *) return 1 ;; esac
}

# Returns 0 if $1 is a valid port number (1–65535)
validate_port() {
    [[ "$1" =~ ^[0-9]+$ ]] && (( $1 >= 1 && $1 <= 65535 ))
}

# Warn once on menu load if config has Match/Include directives.
# This tool only manages plain Host blocks; those lines are preserved
# but will not appear in the server list or be affected by edits.
_check_unsupported_directives() {
    grep -Eqi "^[[:space:]]*(Match|Include)[[:space:]]" "$CONFIG_FILE" 2>/dev/null || return
    echo ""
    echo -e "${BOLD_YELLOW} ⚠️   Notice: ~/.ssh/config contains Match or Include directives.${RESET}"
    echo -e "${BOLD_YELLOW}     This tool only manages plain Host blocks.${RESET}"
    echo -e "${DIM}     Match/Include lines will not be shown or modified.${RESET}"
    echo ""
    sleep 2
}

# get_host_field "hostname" "FieldName" — reads field value from a Host block.
# Match/Include directives are skipped.
get_host_field() {
    local target_host="${1,,}" field="${2,,}" in_block=0 value="" key hname trimmed
    while IFS= read -r line; do
        trimmed="$(echo "$line" | sed 's/^[[:space:]]*//')"
        key="$(echo "$trimmed" | awk '{print tolower($1)}')"
        [[ "$key" == "match" || "$key" == "include" ]] && in_block=0 && continue
        if [[ "$key" == "host" ]]; then
            hname="$(echo "$trimmed" | awk '{print tolower($2)}')"
            in_block=$(( hname == target_host ? 1 : 0 ))
        elif [[ "$in_block" -eq 1 && "$key" == "$field" ]]; then
            value="$(echo "$trimmed" | awk '{$1=""; print substr($0,2)}')"
        fi
    done < "$CONFIG_FILE"
    echo "$value"
}

# load_hosts — populates HOSTS[] from config. Match/Include lines skipped.
load_hosts() {
    HOSTS=()
    while IFS= read -r line; do
        local trimmed
        trimmed="$(echo "$line" | sed 's/^[[:space:]]*//')"
        echo "$trimmed" | grep -Eqi "^(Match|Include)[[:space:]]" && continue
        if echo "$trimmed" | grep -qi "^Host "; then
            HOSTS+=("$(echo "$trimmed" | awk '{print $2}')")
        fi
    done < "$CONFIG_FILE"
}

# remove_host_block "name" — removes a Host block; atomic write via temp file.
# Match/Include directives and all other blocks are preserved.
remove_host_block() {
    local target="${1,,}" in_block=0 output="" trimmed hname
    while IFS= read -r line; do
        trimmed="$(echo "$line" | sed 's/^[[:space:]]*//')"
        if echo "$trimmed" | grep -Eqi "^(Match|Include)[[:space:]]"; then
            in_block=0
            output+="$line"$'\n'
            continue
        fi
        if echo "$trimmed" | grep -qi "^Host "; then
            hname="$(echo "$trimmed" | awk '{print tolower($2)}')"
            if [[ "$hname" == "$target" ]]; then in_block=1; continue; else in_block=0; fi
        fi
        [[ "$in_block" -eq 0 ]] && output+="$line"$'\n'
    done < "$CONFIG_FILE"

    local tmp
    tmp="$(mktemp "$SSH_DIR/.config.tmp.XXXXXX")" || {
        echo -e "  ${BOLD_RED}❌  Failed to create temp file. Config unchanged.${RESET}"
        return 1
    }
    chmod 600 "$tmp"
    printf '%s' "${output%$'\n'}" > "$tmp" && mv "$tmp" "$CONFIG_FILE" || {
        rm -f "$tmp"
        echo -e "  ${BOLD_RED}❌  Failed to write config. Check disk space and permissions.${RESET}"
        return 1
    }
}

# host_exists "name" — returns 0 if alias exists in config
host_exists() {
    local target="${1,,}"
    load_hosts
    for h in "${HOSTS[@]}"; do [[ "${h,,}" == "$target" ]] && return 0; done
    return 1
}

# ip_exists "ip" — returns 0 if HostName already exists in config
ip_exists() {
    grep -Eqi "^[[:space:]]*HostName[[:space:]]+${1,,}[[:space:]]*$" "$CONFIG_FILE"
}
...
main_menu() {
    ...
    case "$choice" in
        ...
        *)
            echo ""
            echo -e "  ${BOLD_RED}❌  Invalid choice.${RESET} Please enter 1, 2, 3, 4 or 5."
            press_enter
            main_menu
            ;;
    esac
...
add_server() {
    ...
    while true; do
        ...
        [[ -z "$new_name" ]] && echo -e "  ${BOLD_RED}❌  Alias cannot be blank.${RESET}" && continue
        break
    done
    ...
    while true; do
        ...
        [[ -z "$new_ip" ]] && echo -e "  ${BOLD_RED}❌  Hostname cannot be blank.${RESET}" && continue
    ...
    if host_exists "$new_name"; then
        ...
        [[ "$dupe_choice" == "1" ]] && do_edit "$new_name" || main_menu
        return
    fi
    ...
    echo -e "  ${DIM}SSH uses port 22 by default. Only change if your server differs.${RESET}"
    echo ""
    local use_port new_port="22"
    ...
    if ask_yes_no "$use_port"; then
        while true; do
            ...
            [[ -z "$new_port" ]] && new_port="22" && break
            validate_port "$new_port" && break
            echo -e "  ${BOLD_RED}❌  Invalid port. Enter a number between 1 and 65535.${RESET}"
        done
    fi
    ...
    echo -e "  ${DIM}Say yes to use a private key, no to log in by password.${RESET}"
    echo ""
    local use_key
...
manual_connect() {
    ...
    while true; do
        ...
        [[ -z "$man_user" ]] && echo -e "  ${BOLD_RED}❌  Username cannot be blank.${RESET}" && continue
        break
    done
    ...
    while true; do
        ...
        [[ -z "$man_ip" ]] && echo -e "  ${BOLD_RED}❌  Hostname cannot be blank.${RESET}" && continue
        break
    done
    ...
    echo ""
    local use_port man_port="22"
    read -rp "$(echo -e "  ${BOLD_WHITE}Use a custom port? (yes/no):${RESET} ")" use_port
    if ask_yes_no "$use_port"; then
        while true; do
            read -rp "$(echo -e "  ${BOLD_WHITE}Port number:${RESET} ")" man_port
            [[ -z "$man_port" ]] && man_port="22" && break
            validate_port "$man_port" && break
            echo -e "  ${BOLD_RED}❌  Invalid port. Enter a number between 1 and 65535.${RESET}"
        done
    fi
    ...
    print_line
    echo ""
    local ssh_args=(-p "$man_port")
    [[ -n "$man_key" ]] && ssh_args+=(-i "$SSH_DIR/$man_key")
    ssh "${ssh_args[@]}" "${man_user}@${man_ip}"
    echo ""
    print_line
    echo -e "  ${BOLD_WHITE}✅  Session ended.${RESET}"
    print_line
    ...
    print_line
    echo -e "  ${BOLD_WHITE}🏷️   Server Alias / Name${RESET}"
    echo -e "  ${DIM}Choose a short nickname to connect by next time. No spaces. Example: vps1${RESET}"
    echo ""
    local man_alias
    while true; do
...
edit_remove() {
    while true; do
        ...
        case "$er_choice" in
            1) do_edit "$er_name"   ; return ;;
            2) do_remove "$er_name" ; return ;;
            3) continue ;;
            *) main_menu ; return ;;
        esac
    done
...
do_edit() {
    ...
    local edit_name edit_ip edit_user edit_port edit_key

    read -rp "$(echo -e "  ${BOLD_WHITE}New alias       ${DIM}(blank = keep '${er_name}'):${RESET} ")" edit_name
    [[ "$edit_name" == "back" || "$edit_name" == "exit" ]] && echo -e "  ${BOLD_YELLOW}⚠️   Edit cancelled.${RESET}" && press_enter_menu && return

    read -rp "$(echo -e "  ${BOLD_WHITE}New IP/hostname ${DIM}(blank = keep '${cur_hostname}'):${RESET} ")" edit_ip
    [[ "$edit_ip" == "back" || "$edit_ip" == "exit" ]] && echo -e "  ${BOLD_YELLOW}⚠️   Edit cancelled.${RESET}" && press_enter_menu && return

    read -rp "$(echo -e "  ${BOLD_WHITE}New username    ${DIM}(blank = keep '${cur_user}'):${RESET} ")" edit_user
    [[ "$edit_user" == "back" || "$edit_user" == "exit" ]] && echo -e "  ${BOLD_YELLOW}⚠️   Edit cancelled.${RESET}" && press_enter_menu && return

    local raw_port
    while true; do
        read -rp "$(echo -e "  ${BOLD_WHITE}New port        ${DIM}(blank = keep '${cur_port}'):${RESET} ")" raw_port
        [[ "$raw_port" == "back" || "$raw_port" == "exit" ]] && echo -e "  ${BOLD_YELLOW}⚠️   Edit cancelled.${RESET}" && press_enter_menu && return
        [[ -z "$raw_port" ]] && edit_port="$cur_port" && break
        if validate_port "$raw_port"; then edit_port="$raw_port"; break; fi
        echo -e "  ${BOLD_RED}❌  Invalid port. Enter a number between 1 and 65535.${RESET}"
    done
    ...
    echo ""
    echo -e "  ${BOLD_GREEN}yes${RESET}   ${DIM}= choose a new key from ~/.ssh${RESET}"
    echo -e "  ${BOLD_RED}no${RESET}    ${DIM}= remove key (log in by password)${RESET}"
    echo -e "  ${DIM}ENTER = keep current key${RESET}"
    echo ""
    print_line
    echo ""
    local key_action
    read -rp "$(echo -e "  ${BOLD_WHITE}Change identity file? (yes/no/Enter to skip):${RESET} ")" key_action
    [[ "$key_action" == "back" || "$key_action" == "exit" ]] && echo -e "  ${BOLD_YELLOW}⚠️   Edit cancelled.${RESET}" && press_enter_menu && return
    ...
    if [[ "${key_action,,}" == "no" ]]; then
        ...
        local picked_key=""
        pick_key "picked_key"
        [[ $? -ne 0 ]] && echo -e "  ${BOLD_YELLOW}⚠️   Edit cancelled.${RESET}" && press_enter_menu && return
    ...
    remove_host_block "$er_name" || { press_enter_menu && return; }
...
do_remove() {
    ...
    print_line
    echo -e "  ${WHITE}Your known_hosts file stores the fingerprint of every server${RESET}"
    echo -e "  ${WHITE}you have connected to. Removing it here keeps things tidy.${RESET}"
    echo ""

    if [[ -z "$remove_ip" ]]; then
        echo -e "  ${BOLD_YELLOW}⚠️   Could not determine server IP — known_hosts cleanup unavailable.${RESET}"
        press_enter_menu
        return
    fi
...
# ── Pick Key ─────────────────────────────────────────────────
# Sets variable named by $1 to the chosen key name (no path).
# Returns 0 on success, 1 on back/exit/cancel.
pick_key() {
    local __result_var="$1" key_count=0 key_names=()
    ...
    print_line
    echo -e "  ${DIM}Names shown without .pub — the private key is used for auth.${RESET}"
    echo ""

    if [[ "$key_count" -eq 0 ]]; then
        echo -e "  ${BOLD_YELLOW}⚠️   No key pairs found in ${SSH_DIR}. Skipping key selection.${RESET}"
        print_line
    ...
    while true; do
        ...
        [[ ! -f "$SSH_DIR/$key_input" ]] && echo -e "  ${BOLD_RED}❌  Key \"${key_input}\" not found in ${SSH_DIR}. Check the name.${RESET}" && continue
        break
    done
